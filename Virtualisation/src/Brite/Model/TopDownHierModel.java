...
public class TopDownHierModel extends BriteModel {
    ...
    {
	if (Models.size()<2) {
	    System.out.println("[BRITE ERROR] Top Down Hierarchical BriteModel requires more than one level.");
	    System.exit(0);
	}
	this.Models = Models;
	...
	String modelParams = "BriteModel ("+ ModelConstants.HI_TOPDOWN+" - TopDown)\n";
	modelParams+= ((BriteModel)Models.get(0)).toString() + ((BriteModel)Models.get(1)).toString();
	return modelParams;
    }
	...
	if (edgeConn == ModelConstants.TD_RANDOM) {
	    ...
	    /*recall that a leaf is a node with degree = BriteModel.m*/
	    int m = ((BriteModel)Models.get(0)).getM();
	...
	Graph g0 = ((BriteModel)Models.get(0)).Generate();  //g0 is toplevel graph (AS)
	...
	if (Models.get(1) instanceof FileModel)
	    FileGraph = ((BriteModel)Models.get(1)).generate();
	*/  
	for (int i=0; i<vec.length; ++i) {
	    ...
	    if (n.getNodeConf() instanceof ASNodeConf)
		{
		    ...
		    Topology t = new Topology((BriteModel)Models.get(1)); //level1 graphs
